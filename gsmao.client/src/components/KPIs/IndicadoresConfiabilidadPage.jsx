...
import {PlantillaGraficaDisponibilidad} from "./PlantillaGraficaDisponibilidad";
...
export const IndicadoresConfiabilidadPage = () => {
    ...
    const fetchData = () => {
        ...
        kpisService.getIndicadoresConfiabilidad(params).then((response) => {
            if (!response.is_error) {
    ...
    // Uso en el useEffect
    useEffect(() => {
        if (data?.graficasPeriodos) {
            setSeries((prev) => ({
                ...prev,
                confiabilidad: data?.graficasPeriodos.map((item) => ({
                    nombrePeriodo: item.nombrePeriodo,
                    valor: (item.confiabilidad && Math.round(item.confiabilidad * 100) / 100) || 0,
                })),
                disponibilidad: data?.graficasPeriodos.map((item) => ({
                    nombrePeriodo: item.nombrePeriodo,
                    valor: (item.disponibilidad && Math.round(item.disponibilidad * 100) / 100) || 0,
                })),
                mttr: data?.graficasPeriodos.map((item) => ({
                    nombrePeriodo: item.nombrePeriodo,
                    valor: item.mttr || 0,
                })),
                mtbf: data?.graficasPeriodos.map((item) => ({
                    nombrePeriodo: item.nombrePeriodo,
                    valor: item.mtbf || 0,
                })),
                ordenesCerradas: data?.graficasPeriodos.map((item) => ({
                    nombrePeriodo: item.nombrePeriodo,
                    valor: item.ordenesCerradas || 0,
                })),
            }));
        }
    }, [data]);
    ...
    const exportTable = async () => {
        ...
        toast.info(t("La exportación de la tabla se está realizando en base a los filtros seleccionados."), {
            position: "top-right",
            autoClose: 10000,
            closeButton: true,
        });
        ...
        const error = await fileDownload(
            `/api/kpis/confiabilidad/descargar-tabla?${params.toString()}`,
            "Desglose_Por_Activos_KPIS_Confiabilidad",
        );
        if (error != undefined) {
            toast.error(`${error}`);
        }
            ...
            {!(series && Object.keys(series).length > 0) ? (
                ...
                <div className="d-flex flex-column gap-3 mt-5">
                    <section className="indicadoresSection">
                        <PlantillaIndicador
                            valorIndicador={data?.totalOrdenesCerradas}
                            tituloIndicador={t("Total Correctivos")}
                            id="indicadorTotalCorrectivosConfiabilidad"
                        />
                        <PlantillaIndicador
                            valorIndicador={data?.totalMTBF || 0}
                            tituloIndicador={t("MTBF")}
                            barraPorcentaje={false}
                            variant="success"
                            unidad="Horas"
                        />
                        <PlantillaIndicador
                            valorIndicador={data?.totalMTTR || 0}
                            tituloIndicador={t("MTTR")}
                            barraPorcentaje={false}
                            unidad="Horas"
                            variant="warning"
                        />
                        <PlantillaIndicador
                            valorIndicador={
                                (data?.totalDisponibilidad && Math.round(data?.totalDisponibilidad * 100) / 100) || 0
                            }
                            tituloIndicador={t("DISPONIBILIDAD")}
                        ...
                        <PlantillaIndicador
                            valorIndicador={
                                (data?.totalConfiabilidad && Math.round(data?.totalConfiabilidad * 100) / 100) || 0
                            }
                            tituloIndicador={t("CONFIABILIDAD")}
                            barraPorcentaje={true}
                            variant="info"
                            unidad="%"
                        />
                    ...
                    <section className="d-flex justify-content-between gap-3 mt-4">
                        ...
                        <div>
                            <CustomButtonIconText
                                ...
                                disabled={isLoading}
                                onClick={() =>
                                    navigate(RoutePaths.EstudioComparativo.path, {state: {idTipoEstudio: 2}})
                                }
                            />
                        </div>
                    </section>
                        ...
                        <div className="contenedorPlantillaCuadrada mt-3">
                            <TableComponent
                                ...
                                columnDefs={[
                                    ...
                                    {
                                        field: "totalOrdenesCerradas",
                                        headerName: t("Total Correctivos Cerrados"),
                                        valueGetter: (e) => e.data?.indicadores?.totalOrdenesCerradas || "-",
                                        flex: 1.5,
                                        minWidth: 230,
                                        editable: true,
                                    },
                                    {
                                        field: "totalMTBF",
                                        headerName: t("MTBF"),
                                        valueGetter: (e) => e.data?.indicadores?.totalMTBF + " Horas" || "-",
                                        flex: 1.5,
                                        minWidth: 120,
                                        editable: false,
                                    },
                                    {
                                        field: "totalMTTR",
                                        headerName: t("MTTR"),
                                        valueGetter: (e) => e.data?.indicadores?.totalMTTR + " Horas" || "-",
                                        flex: 1.5,
                                        minWidth: 120,
                                        editable: false,
                                    },
                                    {
                                        field: "totalDisponibilidad",
                                        headerName: t("Disponibilidad"),
                                        valueGetter: (e) =>
                                            Math.round(e.data?.indicadores?.totalDisponibilidad) + "%" || "-",
                                        flex: 1.5,
                                        minWidth: 120,
                                        editable: false,
                                    },
                                    {
                                        field: "totalConfiabilidad",
                                        headerName: t("Confiabilidad"),
                                        valueGetter: (e) => e.data?.indicadores?.totalConfiabilidad + "%" || "-",
                                        flex: 1.5,
                                        minWidth: 120,
                                        editable: false,
                                    },
